...
def test_gen_diff_plain(paths_json, paths_yaml, paths_mix, result_plain):
    assert generate_diff(*paths_json, format_style='plain') == result_plain
    assert generate_diff(*paths_yaml, format_style='plain') == result_plain
    assert generate_diff(*paths_mix, format_style='plain') == result_plain


def test_gen_diff_json(paths_json, paths_yaml, paths_mix, result_json):
    assert generate_diff(*paths_json, format_style='json') == result_json
    assert generate_diff(*paths_yaml, format_style='json') == result_json
    assert generate_diff(*paths_mix, format_style='json') == result_json
